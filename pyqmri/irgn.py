...
import pyqmri.solver_old as optimizer
...
class IRGNOptimizer:
    ...
    def execute(self, data):
        ...
        if self._streamed:
            data = np.require(
                np.transpose(data, self._data_trans_axes), requirements="C"
            )
        ...
        for ign in range(self.irgn_par["max_gn_it"]):
            ...
            if self._streamed:
                ...
                self._pdop.jacobi = np.sum(np.abs(self._modelgrad) ** 2, 2).astype(
                    self._DTYPE_real
                )
                self._pdop.jacobi[self._pdop.jacobi == 0] = 1e-8
            else:
                _jacobi = np.sum(np.abs(self._modelgrad) ** 2, 1).astype(
                    self._DTYPE_real
                )
    ...
    def _updateIRGNRegPar(self, ign):
        self.irgn_par["delta"] = np.minimum(
            self._delta * self.irgn_par["delta_inc"] ** ign, self.irgn_par["delta_max"]
        )
        self.irgn_par["gamma"] = np.maximum(
            self._gamma * self.irgn_par["gamma_dec"] ** ign, self.irgn_par["gamma_min"]
        )
        self.irgn_par["omega"] = np.maximum(
            self._omega * self.irgn_par["omega_dec"] ** ign, self.irgn_par["omega_min"]
        )
    ...
    def _saveToFile(self, myit, result):
        ...
        if self._reg_type == "TGV":
            ...
            f.create_dataset(
                "tv_result_" + str(myit), result.shape, dtype=self._DTYPE, data=result
            )
    ...
    def _irgnSolve3D(self, x, iters, data, GN_it):
        ...
        if self._streamed:
            ...
            res = (
                data
                - b
                + self._MRI_operator.fwdoop([[x, self._coils, self._modelgrad]])
            )
        else:
            ...
            res = (
                data
                - b
                + self._MRI_operator.fwdoop([tmpx, self._coils, self._modelgrad]).get()
            )
            del tmpx
    ...
    def _calcResidual(self, x, data, GN_it):
        ...
        if self._reg_type == "LOG":
            regcost = np.sum(
                np.abs(np.log(1 + self.irgn_par["gamma"] * np.vdot(grad_tv, grad_tv)))
            )
        elif self._reg_type == "TV":
            regcost = self.irgn_par["gamma"] * np.sum(np.abs(grad_tv))
        else:
            regcost = self.irgn_par["gamma"] * np.sum(
                np.abs(grad_tv - self._v)
        ...
        self._fval = (
            ...
            + regcost
            + L2Cost
            + self.irgn_par["omega"] / 2 * np.linalg.norm(grad_H1) ** 2
        )
        del grad_tv, grad_H1
    ...
    def _calcFwdGNPartLinear(self, x):
        ...
        if self._reg_type == "TGV":
            ...
            sym_grad.add_event(
                self._symgrad_op.fwd(sym_grad, v, wait_for=sym_grad.events + v.events)
            )
    ...
    def _calcFwdGNPartStreamed(self, x):
        ...
        if self._imagespace is False:
            ...
            if self._SMS is True:
                ...
                b = np.require(np.transpose(b, self._data_trans_axes), requirements="C")
            else:
                self._MRI_operator.FTstr.eval(
                    [b],
                    [
