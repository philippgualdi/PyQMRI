...
__kernel void update_ipiano_heavyball_grad_f(
    ...
    __global double2 *AA,
    __global double2 *Ad,
    __global double2 *x_in,
    __global double2 *x_k,
    __global double2 *reg,
    const double dalta,
    const double lambd,
    __global double *min,
    __global double *max,
...
__kernel void update_ipiano_heavyball_fwd(
    __global double2 *x_new,
    __global double2 *x_in,
    __global double2 *x_old,
    __global double2 *Kyk,
    const double alpha,
    const double beta,
    __global double *min,
    ...
    __global int *real, const int NUk)
{
    ...
    for (int uk = 0; uk < NUk; uk++)
    {
        x_new[i] = ((x_in[i] - alpha * Kyk[i] + beta * (x_in[i] - x_old[i])) + tau*x_k[i]) * tauinv;
