...
class IPanoBaseSolver:
    ...
    def run(self, x, xold, data):
        ...
        step_vars_new,
        tmp_results,
        data) = self._setupVariables(x, xold, data)
        ...
        self._update_fwd(out_step_new=step_vars_new,
                         out_tmp = tmp_results,
                         in_step = step_vars)
        ...
        if self.display_iterations:
            if isinstance(step_vars_new["x"], np.ndarray):
                self.model.plot_unknowns(
                    np.swapaxes(step_vars_new["x"], 0, 1))
            else:
    ...
    def update_fwd(self, outp, inp, par, idx=0, idxq=0,
                      bound_cond=0, wait_for=None):
