...
class PDBaseSolver:
    ...
    def update_primal(self, outp, inp, par, idx=0, idxq=0, bound_cond=0, wait_for=None):
    ...
    def update_z1(
        self, outp, inp, par=None, idx=0, idxq=0, bound_cond=0, wait_for=None
    ):
    ...
    def update_r(self, outp, inp, par=None, idx=0, idxq=0, bound_cond=0, wait_for=None):
...
class PDSolverTV(PDBaseSolver):
    ...
    def __init__(
        self, par, irgn_par, queue, tau, fval, prg, linop, coils, model, **kwargs
    ):
        super().__init__(par, irgn_par, queue, tau, fval, prg, coils, model, **kwargs)
        self.alpha = irgn_par["gamma"]
        self._op = linop[0]
...
class PDSolverStreamed(PDBaseSolver):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.unknown_shape = (par["NSlice"], par["unknowns"], par["dimY"], par["dimX"])
        ...
        if imagespace:
            self.data_shape = (par["NSlice"], par["NScan"], par["dimY"], par["dimX"])
    ...
    def _setup_reg_tmp_arrays(self, reg_type, SMS=False):
        if reg_type == "TV":
            ...
            self.v = np.zeros(self.grad_shape, dtype=self._DTYPE)
            self.z2 = np.zeros(self.symgrad_shape, dtype=self._DTYPE)
        else:
            raise NotImplementedError("Not implemented")
        self._setupstreamingops(reg_type, SMS=SMS)

        self.r = np.zeros(self.data_shape, dtype=self._DTYPE)
        self.z1 = np.zeros(self.grad_shape, dtype=self._DTYPE)

    def _setupstreamingops(self, reg_type, SMS=False):
        if not SMS:
            self.stream_initial_1 = self._defineoperator([], [], [[]], reverse_dir=True)
        ...
        if reg_type == "TGV":
            self.stream_Kyk2 = self._defineoperator(
                [self.update_Kyk2],
                [self.grad_shape],
                [[self.symgrad_shape, self.grad_shape, self.grad_shape]],
            )
        ...
        if reg_type == "TGV":
            self.stream_update_v = self._defineoperator(
                [self.update_v], [self.grad_shape], [[self.grad_shape, self.grad_shape]]
            )

            self.update_primal_2 = self._defineoperator([], [], [[]], reverse_dir=True)
            ...
            self.stream_update_z1 = self._defineoperator(
                [self.update_z1_tv],
                [self.grad_shape],
                [[self.grad_shape, self.grad_shape, self.grad_shape]],
                reverse_dir=True,
                posofnorm=[False],
            )
        if not SMS:
            self.stream_update_r = self._defineoperator(
                [self.update_r],
                [self.data_shape],
                [[self.data_shape, self.data_shape, self.data_shape, self.data_shape]],
            )
            self.update_dual_1 = self._defineoperator(
                [], [], [[]], reverse_dir=True, posofnorm=[False, False, True]
            )
        ...
        if reg_type == "TGV":
            self.stream_update_z2 = self._defineoperator(
                [self.update_z2],
                [self.symgrad_shape],
                [[self.symgrad_shape, self.symgrad_shape, self.symgrad_shape]],
            )
    ...
    def _defineoperator(
        self, functions, outp, inp, reverse_dir=False, posofnorm=None, slices=None
    ):
        if slices is None:
            slices = self._NSlice
...
class PDSolverStreamedTGV(PDSolverStreamed):
    ...
    def _setupVariables(self, inp, data):
        ...
        tmp_results_forward = {}
        tmp_results_forward_new = {}
        dual_vars["r"] = np.zeros(data.shape, dtype=self._DTYPE)
        dual_vars_new["r"] = np.zeros_like(dual_vars["r"])

        dual_vars["z1"] = np.zeros(primal_vars["x"].shape + (4,), dtype=self._DTYPE)
        dual_vars_new["z1"] = np.zeros_like(dual_vars["z1"])
        dual_vars["z2"] = np.zeros(primal_vars["x"].shape + (8,), dtype=self._DTYPE)
        dual_vars_new["z2"] = np.zeros_like(dual_vars["z2"])

        tmp_results_forward["gradx"] = np.zeros_like(dual_vars["z1"])
        tmp_results_forward_new["gradx"] = np.zeros_like(dual_vars["z1"])
        tmp_results_forward["symgradx"] = np.zeros_like(dual_vars["z2"])
        tmp_results_forward_new["symgradx"] = np.zeros_like(dual_vars["z2"])
    ...
    def _updateInitial(self, out_fwd, out_adj, in_primal, in_dual):
        ...
        self.stream_initial_1.eval(
            [out_fwd["Ax"], out_adj["Kyk1"], out_fwd["symgradx"]],
            [
        ...
        self.stream_initial_2.eval(
            [out_fwd["gradx"], out_adj["Kyk2"]],
            [[in_primal["x"]], [in_dual["z2"], in_dual["z1"], []]],
            [[], self._symgrad_op.ratio],
        )

    def _updatePrimal(self, out_primal, out_fwd, in_primal, in_precomp_adj, tau):
        self.update_primal_1.eval(
            [out_primal["x"], out_fwd["gradx"], out_fwd["Ax"]],
            [
                [in_primal["x"], in_precomp_adj["Kyk1"], in_primal["xk"], self.jacobi],
                [],
    ...
    def _updateDual(
        ...
        (lhs1, ynorm1) = self.update_dual_1.evalwithnorm(
            [out_dual["z1"], out_dual["r"], out_adj["Kyk1"]],
            [
                [
                    in_dual["z1"],
                    in_precomp_fwd_new["gradx"],
                ...
                [in_dual["r"], in_precomp_fwd_new["Ax"], in_precomp_fwd["Ax"], data],
        ...
        (lhs2, ynorm2) = self.update_dual_2.evalwithnorm(
            ...
            [[beta * tau, theta, self.beta], self._symgrad_op.ratio],
        )
    ...
    def _calcResidual(self, in_primal, in_dual, in_precomp_fwd, in_precomp_adj, data):
        primal_new = (
            self.lambd
            / 2
            * np.vdot(in_precomp_fwd["Ax"] - data, in_precomp_fwd["Ax"] - data)
            + self.alpha * np.sum(abs(in_precomp_fwd["gradx"] - in_primal["v"]))
            + self.beta * np.sum(abs(in_precomp_fwd["symgradx"]))
            + 1
            / (2 * self.delta)
            * np.vdot(
                (in_primal["x"] - in_primal["xk"]) * self.jacobi,
        ...
        dual = (
            -self.delta
            / 2
            * np.vdot(-in_precomp_adj["Kyk1"] / self.jacobi, -in_precomp_adj["Kyk1"])
            - np.vdot(in_primal["xk"], -in_precomp_adj["Kyk1"])
            + np.sum(in_precomp_adj["Kyk2"])
            - 1 / (2 * self.lambd) * np.vdot(in_dual["r"], in_dual["r"])
...
class PDSolverStreamedTGVSMS(PDSolverStreamedTGV):
    ...
    def _updateInitial(self, out_fwd, out_adj, in_primal, in_dual):
        out_fwd["Ax"] = self._op.fwdoop([[in_primal["x"], self._coils, self.modelgrad]])
        self._op.adjKyk1(
            [out_adj["Kyk1"]],
            [[in_dual["r"], in_dual["z1"], self._coils, self.modelgrad, []]],
            [[self._grad_op.ratio]],
...
class PDSolverStreamedTV(PDSolverStreamed):
    ...
    def _updateDual(
        ...
        (lhs1, ynorm1) = self.update_dual_1.evalwithnorm(
            [out_dual["z1"], out_dual["r"], out_adj["Kyk1"]],
            [
                [in_dual["z1"], in_precomp_fwd_new["gradx"], in_precomp_fwd["gradx"]],
                [in_dual["r"], in_precomp_fwd_new["Ax"], in_precomp_fwd["Ax"], data],
                [[], [], self._coils, self.modelgrad, in_precomp_adj["Kyk1"]],
            ],
            [
    ...
    def _calcResidual(self, in_primal, in_dual, in_precomp_fwd, in_precomp_adj, data):
        ...
        primal_new = (
            ...
            / 2
            * np.vdot(in_precomp_fwd["Ax"] - data, in_precomp_fwd["Ax"] - data)
            + self.alpha * np.sum(abs(in_precomp_fwd["gradx"]))
            + 1
            / (2 * self.delta)
            * np.vdot(
                (in_primal["x"] - in_primal["xk"]) * self.jacobi,
            ...
            + clarray.sum(
                abs(
                    clmath.log(
                        1
                        + self.alpha
                        * clarray.vdot(in_precomp_fwd["gradx"], in_precomp_fwd["gradx"])
                    )
                )
            )
            + self.alpha * clarray.sum(abs(in_precomp_fwd["gradx"]))
...
class IPianoBaseSolver:
    ...
    def run(self, x, xold, data):
        ...
        if self.display_iterations:
            if isinstance(step_vars_new["x"], np.ndarray):
                self.model.plot_unknowns(np.swapaxes(step_vars_new["x"], 0, 1))
            else:
                self.model.plot_unknowns(step_vars_new["x"].get())

        # self._calcResidual(step_vars, temp_results, step_vars_new, data)
    ...
    def _calcResidual(self, in_step, temp_results, in_precomp_fwd, data):
        return ({}, {})
    ...
    def updateRegPar(self, irgn_par):
        ...
        ----------
          irgn_par (dic): A dictionary containing the new parameters.
        """
        self.alpha = irgn_par["alpha"]
        self.beta = irgn_par["beta"]
        self.delta = irgn_par["delta"]
        self.omega = irgn_par["omega"]
        self.lambd = irgn_par["lambd"]
        self.mu = 1 / self.delta
    ...
    def update_fwd(self, outp, inp, par, idx=0, idxq=0, bound_cond=0, wait_for=None):
        ...
        return self._prg[idx].update_ipiano_fwd(
            ...
            inp[2].data,
            inp[3].data,
            self._DTYPE_real(par[2]),
            self._DTYPE_real(par[3]),
            self._DTYPE_real(par[0]),
            self._DTYPE_real(par[0] / par[1]),
            self.min_const[idx].data,
            self.max_const[idx].data,
...
class IPianoSolverLog(IPianoBaseSolver):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.beta = 0  # 0.09  # irgn_par["beta"]
        self._op = linop[0]
        self._grad_op = linop[1]
        ...
        out_tmp["gradFx"] = (
            ...
            * abs(
                (
                    ...
                    / (
                        1
                        + self.alpha * clarray.vdot(out_tmp["gradx"], out_tmp["gradx"])
                    )
                ).get()
            )
        )
    ...
    def _calcResidual(self, in_step, temp_results, in_precomp_fwd, data):
        ...
        f_new = (
            clarray.vdot(temp_results["DAd"], temp_results["DAd"])  # self.lambd *
            + clarray.sum(
                abs(
                    clmath.log(
                        1
